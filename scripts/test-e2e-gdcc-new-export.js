...
async function run() {
    console.log('Starting E2E GDCC_new Export and Verification...');
    ...
    try {
        ...
        console.log('Selecting "GDCC_new" template in select dropdown...');
        await page.evaluate(() => {
            const select = document.querySelector('select');
            if (select) {
                const options = Array.from(select.querySelectorAll('option'));
                const gdccOption = options.find(opt => opt.textContent.includes('GDCC_new'));
                if (gdccOption) {
                    select.value = gdccOption.value;
                    select.dispatchEvent(new Event('change', { bubbles: true }));
                    console.log(`Set select dropdown value to "${gdccOption.value}" (GDCC_new)`);
                } else {
                    console.log('GDCC_new option NOT found in select!');
                }
            } else {
        ...
        if (missingVars.length > 0 || base64Count < 9) {
            console.log('Saving failed HTML output to: debug_failed_gdcc_new_report.html');
            fs.writeFileSync(path.join(__dirname, '../debug_failed_gdcc_new_report.html'), domainContent);
        ...
        console.log('✅ VERIFICATION PASSED: All cropped dashboard variables were successfully replaced!');

        // Convert the HTML to Word
        console.log('Calling API to export DOCX...');
        const response = await axios.post(`${BASE_URL}/api/export-docx`, {
            html: domainContent,
            filename: 'gdcc_new_domain_report.docx',
            title: 'GDCC New Domain Report'
        }, {
            responseType: 'arraybuffer'
        });

        const docxBuffer = Buffer.from(response.data);
        const outPath = path.join(__dirname, '../gdcc_new_domain_report.docx');
        fs.writeFileSync(outPath, docxBuffer);
        console.log(`✅ Word document successfully generated and saved to: ${outPath}`);

    } catch (err) {
        console.error('❌ E2E GDCC_new Export Failed:', err.message || err);
        process.exit(1);
    } finally {
        await browser.close();
    }
