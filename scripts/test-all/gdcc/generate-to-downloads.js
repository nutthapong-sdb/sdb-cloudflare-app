const axios = require('axios');
const fs = require('fs');
const path = require('path');
const { log, colors, BASE_URL } = require('../libs/ui-helper');

(async () => {
    log('========================================================', colors.blue);
    log('🚀 Generating GDCC Report & Saving to User Downloads Folder', colors.blue);
    log('========================================================', colors.blue);

    try {
        const userSession = {
            id: 1,
            username: 'root',
            role: 'root',
            cloudflare_api_token: process.env.CLOUDFLARE_API_TOKEN || 'mock-token'
        };

        const payload = {
            userId: '1',
            accountId: '556111fa674b6ccdb9c819dbbf11111b',
            accountName: 'Government Data Center and Cloud service (GDCC)',
            zoneId: 'sesalpglpn.go.th',
            zoneName: 'sesalpglpn.go.th',
            subdomains: ['service.sesalpglpn.go.th'],
            startDate: '2026-06-18',
            endDate: '2026-06-24',
            templateId: '1783484869911',
            promotedHosts: [],
            exportSeparated: false,
            exportThaiDigits: false,
            userSession: userSession
        };

        log(`🔹 Requesting background report job...`, colors.blue);
        const res = await axios.post(`${BASE_URL}/api/gdcc/report-jobs`, payload);
        
        if (!res.data.success) {
            throw new Error(`API failed: ${res.data.message}`);
        }

        const jobId = res.data.data.jobId;
        log(`✅ Job #${jobId} queued.`, colors.green);

        // Poll for job status
        log(`🔹 Polling status of job #${jobId}...`, colors.blue);
        const start = Date.now();
        let finished = false;
        let finalJob = null;

        while (Date.now() - start < 180000) { // 3 minutes timeout
            await new Promise(r => setTimeout(r, 3000));
            const statusRes = await axios.get(`${BASE_URL}/api/gdcc/report-jobs?id=${jobId}`);
            if (statusRes.data.success) {
                const jobList = statusRes.data.data;
                const job = jobList.find(j => j.id === jobId);
                if (!job) {
                    throw new Error(`Job #${jobId} not found in listing!`);
                }
                
                log(`   [Progress] ${job.status.toUpperCase()} (${job.progress}%): ${job.status_message || ''}`, colors.yellow);
                
                if (job.status === 'completed') {
                    finished = true;
                    finalJob = job;
                    break;
                } else if (job.status === 'failed') {
                    throw new Error(`Job failed on server: ${job.error_message}`);
                }
            }
        }

        if (!finished || !finalJob.file_name) {
            throw new Error('Timeout or file name missing from completed job.');
        }

        // Copy file to user Downloads folder
        const sourcePath = path.join(process.cwd(), 'public', 'reports', finalJob.file_name);
        const destDir = '/Users/litarcopperkaikem/Downloads';
        const destPath = path.join(destDir, finalJob.file_name);

        log(`\n🔹 Copying file to Downloads folder...`, colors.blue);
        if (!fs.existsSync(sourcePath)) {
            throw new Error(`Source report file not found at: ${sourcePath}`);
        }

        fs.copyFileSync(sourcePath, destPath);
        log(`✅ Copied to: ${destPath}`, colors.green);

        // Delete job metadata from DB, but KEEP the copied file in Downloads folder!
        // We delete the source file in public/reports via the delete action.
        log(`🔹 Cleaning up database record and temp file...`, colors.blue);
        await axios.get(`${BASE_URL}/api/gdcc/report-jobs?action=delete&id=${jobId}`);
        log(`✅ Cleanup complete.`, colors.green);
        
        log(`\n🎉 Report successfully generated and saved to:`, colors.green);
        log(`   ${destPath}`, colors.cyan);

        process.exit(0);
    } catch (error) {
        log(`❌ Execution Failed: ${error.message}`, colors.red);
        process.exit(1);
    }
})();
