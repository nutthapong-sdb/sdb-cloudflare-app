...
const registryFile = path.join(dataDir, 'ntbc_templates.json');
...
export async function POST(req) {
    try {
        ...
        console.log('NTBC Template API Request:', { action, name, sourceId, id });
        ...
        try {
            ...
            templates = [{ id: 'default', name: 'NTBC Default' }];
        }
        ...
        if (action === 'create' || action === 'duplicate') {
            ...
            if (sourceId === 'empty') {
                ...
                if (sourceId && sourceId !== 'default') {
                    ...
                    if (safeSourceId) {
                        srcSub = path.join(templatesDir, `ntbc_t_${safeSourceId}_sub.json`);
                        srcDomain = path.join(templatesDir, `ntbc_t_${safeSourceId}_domain.json`);
                        srcMiddle = path.join(templatesDir, `ntbc_t_${safeSourceId}_middle.json`);
                    }
                }
            ...
            // Write new files
            await fs.writeFile(path.join(templatesDir, `ntbc_t_${newId}_sub.json`), subContent, 'utf8');
            await fs.writeFile(path.join(templatesDir, `ntbc_t_${newId}_domain.json`), domainContent, 'utf8');
            await fs.writeFile(path.join(templatesDir, `ntbc_t_${newId}_middle.json`), middleContent, 'utf8');
        ...
        if (action === 'delete') {
            ...
            // Delete files
            try { await fs.unlink(path.join(templatesDir, `ntbc_t_${safeId}_sub.json`)); } catch (e) { }
            try { await fs.unlink(path.join(templatesDir, `ntbc_t_${safeId}_domain.json`)); } catch (e) { }
            try { await fs.unlink(path.join(templatesDir, `ntbc_t_${safeId}_middle.json`)); } catch (e) { }
        ...
        console.error('NTBC Template API Error:', error);
