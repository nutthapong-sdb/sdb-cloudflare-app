...
export const createReportJob = async (userId, accountId, accountName, zoneId, zoneName, subdomains, startDate, endDate, templateId = 'default', promotedHosts = [], exportSeparated = false, exportThaiDigits = false, imageWidths = null) => {
    const database = await getGdccDb();
    const query = `
        INSERT INTO gdcc_report_jobs (user_id, account_id, account_name, zone_id, zone_name, subdomains, start_date, end_date, template_id, promoted_hosts, export_separated, export_thai_digits, image_widths, status, progress, status_message)
