...
export const getGdccDb = async () => {
    ...
    try {
      ...
      `);
        ...
        // Migration: Add image_widths column to gdcc_report_jobs if it doesn't exist
        try {
            await db.exec(`ALTER TABLE gdcc_report_jobs ADD COLUMN image_widths TEXT;`);
            console.log('✅ Migrated gdcc_report_jobs: Added image_widths column');
        } catch (e) {
            // Column already exists, safe to ignore
        }

        // Migration: Add page_margins column to gdcc_report_jobs if it doesn't exist
        try {
            await db.exec(`ALTER TABLE gdcc_report_jobs ADD COLUMN page_margins TEXT;`);
            console.log('✅ Migrated gdcc_report_jobs: Added page_margins column');
        } catch (e) {
            // Column already exists, safe to ignore
        }
...
export const createReportJob = async (userId, accountId, accountName, zoneId, zoneName, subdomains, startDate, endDate, templateId = 'default', promotedHosts = [], exportSeparated = false, exportThaiDigits = false, imageWidths = null, pageMargins = null) => {
    const database = await getGdccDb();
    const query = `
        INSERT INTO gdcc_report_jobs (user_id, account_id, account_name, zone_id, zone_name, subdomains, start_date, end_date, template_id, promoted_hosts, export_separated, export_thai_digits, image_widths, page_margins, status, progress, status_message)
    ...
    const result = await database.run(query, [
        ...
        imageWidths,
        pageMargins ? (typeof pageMargins === 'string' ? pageMargins : JSON.stringify(pageMargins)) : null
    ]);
