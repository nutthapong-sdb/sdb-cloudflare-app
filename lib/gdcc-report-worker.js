...
export async function runBackgroundReportJob(jobId, baseUrl, userSession) {
    // Resolve internal loopback URL to bypass Docker external DNS/NAT issues
    const localPort = process.env.PORT || '8002';
    const localBaseUrl = `http://127.0.0.1:${localPort}`;
    console.log(`🤖 [Background Worker] Starting report job #${jobId} (using local loopback: ${localBaseUrl})...`);
    ...
    try {
        ...
        await page.goto(`${localBaseUrl}/login`, { waitUntil: 'domcontentloaded', timeout: 60000 });
        
        await page.evaluate((sess, widths, margins) => {
            ...
            if (widths) {
                localStorage.setItem('gdcc:cropped-image-widths', widths);
            }
            if (margins) {
                localStorage.setItem('gdcc:page-margins', margins);
            }
        }, userSession, job.image_widths, job.page_margins);
        ...
        await page.goto(`${localBaseUrl}/systems/gdcc?mode=worker`, { waitUntil: 'networkidle2', timeout: 60000 });
        
        // 6. Wait for window.__handleBatchReport hook to be ready
        await page.waitForFunction(() => typeof window.__handleBatchReport === 'function', { timeout: 60000 });
        ...
        let isDone = false;
        let lastKnownStatus = 'Initializing report generation...';
        
        // Dynamically scale the timeout: 1 minute per subdomain, with a minimum of 5 minutes (300,000 ms)
        const maxTimeoutMs = Math.max(300000, subdomains.length * 60000);
        console.log(`🤖 [Background Worker] Polling timeout set to ${maxTimeoutMs / 1000}s (${maxTimeoutMs / 60000} minutes) for ${subdomains.length} subdomains.`);
        ...
        while (Date.now() - startTime < maxTimeoutMs) {
            ...
            if (progressInfo) {
                ...
                lastKnownStatus = `Processing ${progressInfo.index}/${progressInfo.total} (${progressInfo.hostName}): ${progressInfo.statusMsg}`;
                await updateReportJobProgress(jobId, scaledProgress, 'processing', lastKnownStatus);
            }
        ...
        if (!isDone) {
            throw new Error(`Timeout: Report generation took too long or page stalled (Last Status: ${lastKnownStatus})`);
        }
        ...
        if (job.export_separated === 1) {
            ...
            let parsedMargins = null;
            if (job.page_margins) {
                try {
                    parsedMargins = JSON.parse(job.page_margins);
                } catch (e) {}
            }

            console.log(`🤖 [Background Worker] Converting HTML to DOCX via local API (margins: ${job.page_margins || 'default'})...`);
            const response = await axios.post(`${localBaseUrl}/api/export-docx`, {
                html: htmlContent,
                filename: fileName,
        ...
        // Take screenshot of the browser page when error/timeout occurs to help debugging
        if (page) {
            try {
                const reportsDir = path.join(process.cwd(), 'public', 'reports');
                if (!fs.existsSync(reportsDir)) {
                ...
                const screenshotPath = path.join(reportsDir, `error_screenshot_${jobId}.png`);
                await page.screenshot({ path: screenshotPath });
                console.log(`📸 [Background Worker] Saved error screenshot to ${screenshotPath}`);
            } catch (screenshotErr) {
                console.error('❌ [Background Worker] Failed to take error screenshot:', screenshotErr);
            }
        }
